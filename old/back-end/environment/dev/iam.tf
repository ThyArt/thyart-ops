...
resource "aws_iam_access_key" "dev-user-access-key" {
  user = "${aws_iam_user.dev-user.name}"
}

resource "aws_iam_user_policy" "dev-user-policy" {
  name = "${var.application_name}-dev-user-policy"
  user = "${aws_iam_user.dev-user.name}"
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "${aws_s3_bucket.dev-image-bucket.arn}"
            ]
        },
        {
            "Effect": "Allow",
