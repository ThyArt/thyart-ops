...
resource "random_string" "codebuild-rds-database-username" {
  length  = 8
  special = false
  upper   = false
}

resource "random_string" "codebuild-rds-database-name" {
  length  = 8
  special = false
  upper   = false
  number  = false
...
resource "aws_db_instance" "codebuild-rds-database" {
  ...
  name                    = "${random_string.codebuild-rds-database-name.result}"
  username                = "${random_string.codebuild-rds-database-username.result}"
