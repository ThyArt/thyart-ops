resource "random_string" "prod-passport-client-secret" {
  length  = 40
  special = false
}
...
resource "aws_elastic_beanstalk_environment" "prod-environment" {
  ...
  setting {
    ...
    name      = "RDS_USERNAME"
    value     = "${aws_db_instance.prod-rds-database.username}"
  }

  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "RDS_PASSWORD"
    value     = "${aws_db_instance.prod-rds-database.password}"
  }

  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "RDS_DB_NAME"
    value     = "${aws_db_instance.prod-rds-database.name}"
  }

  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "RDS_HOSTNAME"
    value     = "${aws_db_instance.prod-rds-database.address}"
  }

  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "RDS_PORT"
    value     = "${aws_db_instance.prod-rds-database.port}"
  }
